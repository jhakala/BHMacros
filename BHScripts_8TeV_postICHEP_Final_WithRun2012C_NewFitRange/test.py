...
def main():
   ...
   from ROOT import TFile, TCanvas, THStack, TLegend, TPaveText, gStyle, TMath
   ...
   #pulls
   hPull = infile.Get("Background_N%d%s" % (N, suffix))
   for ibin in range(hData.GetNbinsX()):
      if hBkg.GetBinContent(ibin+1) != 0 and hData.GetBinContent(ibin+1) != 0:
         cont = hData.GetBinContent(ibin+1)/hBkg.GetBinContent(ibin+1) - 1
         cont_err =  cont*TMath.Sqrt(1/hData.GetBinContent(ibin+1) + (hBkg.GetBinError(ibin+1)/hBkg.GetBinContent(ibin+1))*(hBkg.GetBinError(ibin+1)/hBkg.GetBinContent(ibin+1)))
         print ibin+1,hData.GetBinContent(ibin+1), hBkg.GetBinContent(ibin+1), hBkg.GetBinError(ibin+1),hData.GetBinContent(ibin+1)/hBkg.GetBinContent(ibin+1) - 1,TMath.Sqrt(1/hData.GetBinContent(ibin+1) + (hBkg.GetBinError(ibin+1)/hBkg.GetBinContent(ibin+1))*(hBkg.GetBinError(ibin+1)/hBkg.GetBinContent(ibin+1))) 
