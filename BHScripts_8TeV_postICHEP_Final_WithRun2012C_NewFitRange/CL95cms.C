...
static Int_t N, MaxIterations = 1000;
...
Double_t CL95(Double_t ilum, Double_t slum, Double_t eff, Double_t seff, Double_t bck, Double_t sbck, Int_t n, Bool_t gauss, Int_t nuisanceModel)
{
	...
	if (!gauss) 
	{
		...
		else if (nuisanceModel == 1) // Lognormal distribution is used
		{
			if (B0 <= 0.)
			{
				cout << endl << "*** Warning: lognormal distribution is ill-defined for B = 0.! ***" << endl;
				B0 = sB/1.E3;
				if (B0 == 0.) cout << "*** Error: both background and its uncertainty can't be 0! ***" << endl;
			}
			sigma_b = TMath::Log(1. + sB/B0);
			if (A0 > 0) sigma_a = TMath::Log(1. + sA/A0);
			else cout << "Error: efficiency times luminosity can't be 0!";
		...
		else if (nuisanceModel == 2) // Gamma distribution is used
		{
			...
			if (sB > 0) 
			{
				if (B0 <= 0.)
				{
					cout << endl << "*** Warning: Gamma distribution is ill-defined for B = 0.! ***" << endl;
					B0 = sB/1.E3;
				}
...
//		if (plot) cout << "delta = " << delta << endl; 
	...
	{      
		...
		{
			cout << "Maximum number of iterations reached (" << timeout << "); the following precision has been achieved: " << delta << endl;
			delta = 0.;
		}
	}         
	xmax = x[0];
...
// (N is a global parameter. The parameter p is dummy).
{
	...
	if (sB == 0.)
	{
		...
		else 
		{
			...
			Double_t par[2];
			TF1 *in = new TF1("Inner",Inner,0.,TMath::Max(A0 + 5.*sA,1.E10),2);
	...
	else
	{
		...
		TF1 *out = new TF1("Outer",Outer,0.,TMath::Max(B0 + 5.*sB,1.E10),1);   
		if (I == 0)
		{
...
// This function can make use of function Inner, which also takes into account uncertainty in lumi*effi.
{
	...
	else 
	{
		...
		{
			Double_t low = A0 > 5.*sA ? A0 - 5.*sA : 0.;
			retval = TMath::Gaus(x[0],B0,sB,kTRUE)*in->Integral(low,A0+5.*sA,par,epsilon);
		} else {
			Double_t low = 0., high = A0 + 5.*sA, tmp = 1.;
			while (tmp > epsilon)
			{	 
			...
			if (I == 1) retval *= TMath::LogNormal(x[0], sigma_b, 0., B0);		
			else if (I == 2) retval *= TMath::GammaDist(x[0], sigma_b, 0., 1./tau_b);
			else retval = 0;
		}		
		delete in;
	}
...
{
	plot = kFALSE;
	Double_t CL95A = 0, precision = 1.e-3;
	Int_t i;
	for (i = bck; i >= 0; i--)
	{
		//
